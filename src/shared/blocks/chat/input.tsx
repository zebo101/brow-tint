    ...
    <div className="w-full">
      <PromptInput
        onSubmit={async (message) => {
          try {
            handleSubmitAction(message, { model, webSearch, reasoning });
            setInput('');
          } catch (err) {
            // Allow parent to control error display/state. Do not clear input.
          }
        }}
        className="mt-4 [&>div]:overflow-hidden [&>div]:rounded-2xl"
        globalDrop
        multiple
      >
        {/* <PromptInputHeader>
        ...
        <PromptInputBody>
          <PromptInputTextarea
            className="min-h-[60px] overflow-hidden p-4 ring-0 focus-visible:ring-0 focus-visible:ring-offset-0"
        ...
        <PromptInputFooter className="bg-background">
          ...
          <PromptInputSubmit
            disabled={
              status === 'submitted' || status === 'streaming' ? false : !input
            }
            status={status}
          />
        </PromptInputFooter>
