...
import { getBrowTints, type BrowTint } from '@/shared/models/brow-tint';
import type { Section } from '@/shared/types/blocks/landing';

type BrowTintCategoryConfig = {
  key: string;
  title: string;
  description?: string;
  cta_label?: string;
...
interface BrowTintGallerySection extends Section {
  limit?: number;
  cta_url?: string;
  categories?: BrowTintCategoryConfig[];
}

async function loadCategory(key: string, limit: number): Promise<BrowTint[]> {
  try {
    return await getBrowTints({ category: key, status: 'active', limit });
  } catch (error) {
    console.warn(`[brow-tint-gallery] failed to load "${key}":`, error);
    return [];
  }
}
 ...
 */
function buildDisplayNames(items: BrowTint[]): Map<string, string> {
...
export async function BrowTintGallery({
  section,
  className,
}: {
  section: BrowTintGallerySection;
  className?: string;
}) {
  ...
  return (
    <section
      id={section.id || 'brow-tint-gallery'}
